...
export default function SideMenu({ moduleList, collapseListID }) {
  ...
  const AddLectureInModule = ({ lectures, num }) => {
    ...
    return lectures.map(el => (
      <div id={`collapse_${num}`} className="collapse show" aria-labelledby="headingOne" data-parent="#accordionExample">
        <ul class="list-group">
      ...
      <div className="accordion" id="accordionExample">
        <div className="card">
          <div className="card-header" id="headingOne">
            <h2 className="mb-0">
              <button className="btn btn-link" type="button" data-toggle="collapse" data-target={`#collapse_${num}`} aria-expanded="true" aria-controls={"collapse_" + { num }}>
                {el.moduleName}
              </button>
            </h2>
          </div>
